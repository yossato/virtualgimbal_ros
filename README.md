...
|lsm_period|double|1.5|VirtualGimbalの動画安定化アルゴリズムは、時系列方向に平滑化したカメラ角度と、平滑化していない生のカメラ角度の差の情報から、画像のシフト量を計算している。このときの時系列の平滑化方法として、VirtualGimbal ROSでは、最小二乗法を採用した。このlsm_periodは、最小二乗法(least squares method)を計算する時間の長さ(秒)である。値を大きくすると動画の安定化能力が向上するが、急なカメラの動きに追従できなくなる。値を小さくすると安定化能力が低下するがカメラの急な動きに追従できるようになる。|
